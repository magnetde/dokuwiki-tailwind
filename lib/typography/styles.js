const merge = require('../util/mergedeep');
const apply = require('../util/apply');
const dark = require('../util/dark');
...
const proseExtension = (theme) => ({
    DEFAULT: {
        css: {
            ...
            '--tw-prose-dead-links': theme('colors.red.700'),
            ...
            '--tw-prose-invert-dead-links': theme('colors.red.700'),
            ...
            h1: apply`mt-12 first:mt-0`,
            h5: {
                ...apply`mt-6 mb-2 leading-6`,
                ...apply`text-[--tw-prose-headings] font-medium`,
                'strong': apply`text-inherit font-semibold`,
                'code': apply`text-inherit`,
                '& + *': apply`mt-0`,
            },
            p: apply`first:mt-0 last:mb-0`,
            ul: apply`my-2`,
            li: apply`my-1`,
            // Underline text
            ...
            a: {
                // Regular links
                '&.wikilink1, &.urlextern, &.mail, &.windows, &.mediafile, &.interwiki': merge(
                    link('--tw-prose-links'),
                    apply`[font-weight:inherit]`,
                ),
                // Dead links
                '&.wikilink2': merge(
                    link('--tw-prose-dead-links'),
                    apply`[font-weight:inherit]`,
                ),
                ...
                '&.interwiki': {
                    ...
                    '&.iw_wp, &.iw_wpde, &.iw_wpes, &.iw_wpfr, &.iw_wpjp, &.iw_wpmeta, &.iw_wppl': {
                        ...
                        ...apply`
                            ...
                            dark:bg-[url('/lib/tpl/tailwind/icon.php?icon=wikipedia&color=white')]
                        `,
                    },
                },
            ...
            '.section-header': {
                ...
                '.secedit': {
                    ...
                    '.btn_secedit': {
                        ...
                        button: {
                            ...apply`
                                inline-block align-middle bg-no-repeat bg-contain bg-center
    ...
    invert: {
        css: {
            ...
            '--tw-prose-dead-links': 'var(--tw-prose-invert-dead-links)',
        },
    },
    // Sidebar fix
    sm: {
        css: {
            h1: apply`mt-10 first:mt-0`,
            h5: apply`mt-5 mb-1.5 leading-6`,
            ...listFix,
        }
    },
    // Adds print styles
    print: {
        css: {
            a: {
                ...apply`!text-inherit`,
                // Regular links
                '&.wikilink1, &.urlextern, &.mail, &.windows, &.mediafile, &.interwiki': apply`underline`,
                // Dead links
                '&.wikilink2': apply`no-underline`,
                // Special links with icons
                '&.urlextern, &.mail, &.windows, &.mediafile, &.interwiki': apply`pl-0 ml-0 !bg-none`,
                // Footnotes (in text)
                '&.fn_top': apply`no-underline`,
            },
            // Code
            code: apply`text-gray-600 bg-transparent`,
            pre: {
                '&.code, &.file': {
                    ...apply`text-inherit bg-transparent border-2 border-gray-200 whitespace-break-spaces`,

                    // Disable code highlighting when printing
                    span: apply`text-inherit`,
                }
            },
            'dl.code, dl.file': {
                dt: {
                    ...apply`border-2 border-b-0 bg-transparent`,
                    '> a': apply`hidden`,
                },
                dd: apply`border-0`,
            },
            hr: apply`border-t-2`,
            'abbr:where([title])': apply`no-underline`,
            // Footnotes (at the bottom)
            '.footnotes': {
